...
class SimpleReactionListDataModule(_AbsDataModule):
    ...
    def __init__(self, augment_prob: float = 0.0, reverse: bool = False, batch_first: bool = False, **kwargs):
        super().__init__(**kwargs)
        self._batch_augmenter = SMILESAugmenter(augment_prob=augment_prob)
        self._encoder = BatchEncoder(
            tokenizer=self.tokenizer, masker=None, max_seq_len=self.max_seq_len, batch_first=batch_first
        )
