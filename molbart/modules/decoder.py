...
class BeamSearchSampler:
    ...
    def sample_molecules(
        ...
        batch_size = batch_input["encoder_input"].shape[0 if self.batch_first else 1]
...
class DecodeSampler:
    def __init__(self, tokenizer, max_seq_len, length_norm=None, batch_first: bool = False):
        self.tokenizer = tokenizer
    ...
    def greedy_decode(self, decode_fn, batch_size, device="cpu"):
        ...
        for i in range(1, self.max_seq_len):
            ...
            if self.batch_first:
                output_dist = decode_fn(token_ids_seq.transpose(0, 1), pad_mask_seq.transpose(0, 1))
                output_dist.transpose(0, 1)
            else:
    ...
    def _beam_step(self, decode_fn, tokens, mask, lls):
        ...
        # Sample next id for each element in the batch
        if self.batch_first:
            output_dist = decode_fn(tokens.transpose(0, 1), mask.transpose(0, 1))
            output_dist.transpose(0, 1)
        else:
    ...
    def compute_sampling_metrics(sampled_smiles, target_smiles):
        ...
        The the number of invalid SMILES "fraction_invalid" is also returned
            (for beam search this is just from the top_1)
