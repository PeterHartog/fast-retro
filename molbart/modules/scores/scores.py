...
class TopKAccuracyScore(BaseScore):
    ...
    def __init__(
        self,
        top_ks: np.ndarray = np.array([1, 2, 3, 5, 10, 20, 30, 40, 50]),
...
class TopKValidAccuracyScore(BaseScore):
    scorer_name = "top_k_valid_accuracy"
    ...
    def _is_in_set(self, sampled_smiles: List[List[str]], target_smiles: List[str], k: int) -> np.ndarray:
        if not self._canonicalized:
            ...
            sampled_smiles = [
                [smiles_utils.canonicalize_smiles(smiles, return_non_valid=False) for smiles in smiles_list]
                for smiles_list in sampled_smiles
            ]
            sampled_smiles = [[x for x in l if x is not None] for l in sampled_smiles]
            sampled_smiles = [l + [None] * (k-len(l)) for l in sampled_smiles]
    ...
    def _score_sampled_smiles(self, sampled_smiles: List[List[str]], target_smiles: List[str]) -> Dict[str, float]:
        ...
        is_in_set = np.zeros((len(sampled_smiles), len(top_ks)), dtype=bool)
        for i_k, k in enumerate(top_ks):
            columns.append(f"accuracy_valid_top_{k}")
            is_in_set[:, i_k] = self._is_in_set(sampled_smiles, target_smiles, k)
        ...
        if max(top_ks) == 1:
            return {"accuracy_valid": top_n_accuracy[0]}

        scores = {col: accuracy for col, accuracy in zip(columns, top_n_accuracy)}
        return scores


class TopKValidUniqueAccuracyScore(BaseScore):
    scorer_name = "top_k_valid_unique_accuracy"
    ...
    def _is_in_set(self, sampled_smiles: List[List[str]], target_smiles: List[str], k: int) -> np.ndarray:
        if not self._canonicalized:
            ...
            sampled_smiles = [[x for x in l if x is not None] for l in sampled_smiles]
            sampled_smiles = [list(dict.fromkeys(l)) for l in sampled_smiles]
            sampled_smiles = [l + [None] * (k-len(l)) for l in sampled_smiles]
    ...
    def _score_sampled_smiles(self, sampled_smiles: List[List[str]], target_smiles: List[str]) -> Dict[str, float]:
        ...
        is_in_set = np.zeros((len(sampled_smiles), len(top_ks)), dtype=bool)
        for i_k, k in enumerate(top_ks):
            columns.append(f"accuracy_valid_unique_top_{k}")
            is_in_set[:, i_k] = self._is_in_set(sampled_smiles, target_smiles, k)
        ...
        if max(top_ks) == 1:
            return {"accuracy_valid_unique": top_n_accuracy[0]}
